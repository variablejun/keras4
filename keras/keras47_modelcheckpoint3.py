...
date_time = date.strftime("%m%d_%H%M") # 시간 부분편집
filepath = './_save/ModelCheckpoint/'
filename = '.{epochs:04d}-{val_loss:.4f}.hdf5'
modelpath = "".join([filepath, "k47_",date_time,"_",filename]) #합치기
mcp = ModelCheckpoint(monitor='val_loss', save_best_only=True,mode='auto', verbose=1,filepath=modelpath)
