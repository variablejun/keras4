...
model = Sequential()
               #라벨의개수(단어사전의수)   CNN필터,DNN RNN유닛   문장의 길이
model.add(Embedding(input_dim=27, output_dim=77, input_length=5))# 벡터화(수치화)
# 원핫인코딩으로 0을채우면 쓸모없는 데이터가 많아지기 때문이다.
# input을 크게준다고 오류가 나진 않지만 연산수가 많아지기때문에 단어사전 수 만큼 넣어준다
# 벡터화 해서 수치화 하기 때문에 input_lenth가 큰 의미를 가지지않는다.
#model.add(Embedding(27,77,input_length=5))위 문장과 같다 inputlength는 고정되어있다고 판단
...
embedding (Embedding)        (None, 5, 77)             2079
_________________________________________________________________
bidirectional (Bidirectional (None, 64)                28160
_________________________________________________________________
