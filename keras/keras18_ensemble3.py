...
x1_train, x1_test, y1_train, y1_test, y2_train, y2_test = train_test_split(x1,y1,y2,train_size = 0.7, random_state=66) # train_size 0.7
...
dence4 = Dense(7, activation='relu')(dence3)
dence5 = Dense(5, activation='relu')(dence4)
output1 = Dense(3)(dence5)


# 2-2
# 실제 아웃풋이 아니라 활성화함수써됨 마지막 레이어에서는 따로 사용하는 default 활성화함수 사용
# output이 나왔지만 히든레이어
...
from tensorflow.keras.layers import Concatenate
 # concatenate는 양쪽 모델에서 나온 아웃풋을 하나로묶어준다.

output21 = Dense(7)(output1)# 원하는 레이어를 붙여주기만 하면 거기에서 바로 분기시킬 수 있다
last_output1 = Dense(1)(output21) 

output22 = Dense(7)(output1)
last_output2 = Dense(1)(output22)
# concatenate 를 안하고 output에서 분기만 잡는다.
model = Model(inputs = input1, outputs = [last_output1,last_output2])
model.summary()
model.compile(loss = 'mse', optimizer = 'adam', metrics=['mae'])
model.fit(x1_train,[y1_train,y2_train ], epochs=100, batch_size=10, validation_split=0.3, shuffle=0.2) 
# model.fit([x1,x2],y, epochs=100, batch_size=10, validation_split=0.3, shuffle=0.2) 

loss = model.evaluate(x1_test, [y1_test, y2_test]) 
print(loss)
#[2275947.75, 1033867.875, 1242079.875, 1016.3628540039062, 1114.093994140625] 5 개가 나온다
# loss: 2275947.7500 - dense_7_loss: 1033867.8750 - dense_9_loss: 1242079.8750 - dense_7_mae: 1016.3629 - dense_9_mae: 1114.0940
print('loss : ', loss[0])
